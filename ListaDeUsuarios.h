...
public:
	...
	void OrdenarUsuarios() {
		function<bool(Nodo<Usuario>*, Nodo<Usuario>*)>  condicion = [](Nodo<Usuario>* j, Nodo<Usuario>* i) {return i->getelemento().getNombreUsuario()[0] > j->getelemento().getNombreUsuario()[0]; };
	...
	void EliminarPrimerUsuario() {
		function<bool(Nodo<Usuario>*)>  condicion = [](Nodo<Usuario>* n) {return n->getelemento().getNombreUsuario() != "admin"; };
		usuarios.setinicio(usuarios.deleteFirstNodo(usuarios.getfin(),condicion));
		MostrarUsuarios();
	}
	
	void Mostrar_UsuariosAdmin(){ //e
	...
	void Encontrar_imprimirUsuario_lambda() { //e
		int pos;
		cout << "Ingrese la posicion del usuario en la lista: ";
		cin >> pos;

		function<void(Usuario)> imprime = [](Usuario n) { cout << n.toString() << endl; };
		usuarios.Encontrar_Usuariopos(pos, imprime);
	}
	
	void EliminarUltimoUsuario() { //e
		function<bool(Nodo<Usuario>*)>  condicion = [](Nodo<Usuario>* u) {return u->getelemento().getNombreUsuario() != "admin"; };

		usuarios.setfin(usuarios.Elim_UltimoUsuario(usuarios.getfin(), condicion));
		MostrarUsuarios();
	}
